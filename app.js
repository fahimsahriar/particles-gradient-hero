...
var pJS = function (e, a) {
    ...
    a && Object.deepExtend(i, a), i.tmp.obj = {
        ...
        for (var e = 0; e < i.particles.array.length; e++) {
            ...
            if (a.x - a.radius > i.canvas.w ? (a.x = n.x_left, a.y = Math.random() * i.canvas.h) : a.x + a.radius < 0 && (a.x = n.x_right, a.y = Math.random() * i.canvas.h), a.y - a.radius > i.canvas.h ? (a.y = n.y_top, a.x = Math.random() * i.canvas.w) : a.y + a.radius < 0 && (a.y = n.y_bottom, a.x = Math.random() * i.canvas.w), "bounce" === i.particles.move.out_mode) (a.x + a.radius > i.canvas.w || a.x - a.radius < 0) && (a.vx = -a.vx), (a.y + a.radius > i.canvas.h || a.y - a.radius < 0) && (a.vy = -a.vy);
...
}, window.cancelRequestAnimFrame = window.cancelAnimationFrame || window.webkitCancelRequestAnimationFrame || window.mozCancelRequestAnimationFrame || window.oCancelRequestAnimationFrame || window.msCancelRequestAnimationFrame || clearTimeout, window.pJSDom = [],
...
/* Responsive particles initializer: adjusts particle count and density by screen width */
(function(){
    var configs = {
        'particles-js': particlesConfig1,
        'particles-js2': particlesConfig2,
        'particles-js3': particlesConfig3,
        'particles-js4': particlesConfig4,
        'particles-js5': particlesConfig5,
        'particles-js6': particlesConfig6,
        'particles-js7': particlesConfig7,
        'particles-js8': particlesConfig8
    };

    function getScaleForWidth(w){
        if(w <= 480) return 0.5;
        if(w <= 768) return 0.75;
        if(w <= 1024) return 0.9;
        if(w <= 1280) return 1;
        if(w <= 1600) return 1.25;
        if(w <= 1920) return 1.5;
        if(w <= 2560) return 1.8;
        if(w <= 3840) return 2.2;
        return 2.5;
    }

    function adjustConfig(baseCfg, scale){
        var cfg = JSON.parse(JSON.stringify(baseCfg));
        try{
            if(cfg && cfg.particles && cfg.particles.number){
                var baseNumber = (baseCfg.particles.number && baseCfg.particles.number.value) ? baseCfg.particles.number.value : 10;
                var baseDensity = (baseCfg.particles.number && baseCfg.particles.number.density && baseCfg.particles.number.density.value_area) ? baseCfg.particles.number.density.value_area : 800;
                var adjustedNumber = Math.max(1, Math.round(baseNumber * scale));
                var densityArea = Math.round(baseDensity * Math.max(1, 1 / scale));
                cfg.particles.number.value = adjustedNumber;
                if(cfg.particles.number.density) cfg.particles.number.density.value_area = densityArea;
            }
        }catch(e){
            console && console.warn && console.warn('adjustConfig error', e);
        }
        return cfg;
    }
    ...
    function initResponsiveParticles(){
        ...
        var scale = getScaleForWidth(w);
        for(var id in configs){
            if(!configs.hasOwnProperty(id)) continue;
            try{
                var cfg = adjustConfig(configs[id], scale);
                particlesJS(id, cfg);
            }catch(e){
                console && console.warn && console.warn('particlesJS init failed for', id, e);
            }
        }
    }
