/* -----------------------------------------------
         * Modified particles.js Library (Inline)
         * Optimized for per-instance FPS limiting
         * ----------------------------------------------- */
...
var pJS = function (tag_id, params) {
    ...
    // Map the root param fpsLimit to internal variable
    if (params.fpsLimit) pJS.fps_limit = params.fpsLimit;
    ...
    pJS.fn.particle = function (color, opacity, position) {
        ...
        if (typeof (color.value) == 'object') {
            ...
            this.color.rgb = { r: (Math.floor(Math.random() * (255 - 0 + 1)) + 0), g: (Math.floor(Math.random() * (255 - 0 + 1)) + 0), b: (Math.floor(Math.random() * (255 - 0 + 1)) + 0) }
        }
    ...
    pJS.fn.particle.prototype.draw = function () {
        var p = this;
        var radius = (p.radius_bubble != undefined) ? p.radius_bubble : p.radius;
        var opacity = (p.opacity_bubble != undefined) ? p.opacity_bubble : p.opacity;
        ...
        switch (p.shape) {
            ...
            case 'image':
                var img_obj = (pJS.tmp.img_type == 'svg') ? p.img.obj : pJS.tmp.img_obj;
                if (img_obj) pJS.canvas.ctx.drawImage(img_obj, p.x - radius, p.y - radius, radius * 2, radius * 2 / p.img.ratio);
                break;
        }
    ...
    pJS.fn.particlesUpdate = function () {
        for (var i = 0; i < pJS.particles.array.length; i++) {
            ...
            if (pJS.particles.opacity.anim.enable) {
                if (p.opacity_status == true) { if (p.opacity >= pJS.particles.opacity.value) p.opacity_status = false; p.opacity += p.vo; }
                else { if (p.opacity <= pJS.particles.opacity.anim.opacity_min) p.opacity_status = true; p.opacity -= p.vo; }
                if (p.opacity < 0) p.opacity = 0;
            }
            if (pJS.particles.size.anim.enable) {
                if (p.size_status == true) { if (p.radius >= pJS.particles.size.value) p.size_status = false; p.radius += p.vs; }
                else { if (p.radius <= pJS.particles.size.anim.size_min) p.size_status = true; p.radius -= p.vs; }
                if (p.radius < 0) p.radius = 0;
            }
            var new_pos = (pJS.particles.move.out_mode == 'bounce') ? { x_left: p.radius, x_right: pJS.canvas.w, y_top: p.radius, y_bottom: pJS.canvas.h } : { x_left: -p.radius, x_right: pJS.canvas.w + p.radius, y_top: -p.radius, y_bottom: pJS.canvas.h };
    ...
    pJS.fn.interact = {
        ...
        attractParticles: function (p1, p2) {
            var dx = p1.x - p2.x, dy = p1.y - p2.y, dist = Math.sqrt(dx * dx + dy * dy);
            if (dist <= pJS.particles.line_linked.distance) {
        ...
        bounceParticles: function (p1, p2) {
                ...
                function process(bubble_param, particles_param, p_obj_bubble, p_obj, id) {
                    if (bubble_param != particles_param) {
                        if (!pJS.tmp.bubble_duration_end) {
                            if (dist_mouse <= pJS.interactivity.modes.bubble.distance) {
                                var obj = (p_obj_bubble != undefined) ? p_obj_bubble : p_obj;
                                if (obj != bubble_param) {
    ...
    pJS.fn.vendors.eventsListeners = function () {
        ...
        if (pJS.interactivity.events.onhover.enable || pJS.interactivity.events.onclick.enable) {
            pJS.interactivity.el.addEventListener('mousemove', function (e) {
                ...
                pJS.interactivity.mouse.pos_x = pos_x; pJS.interactivity.mouse.pos_y = pos_y;
        ...
        if (pJS.interactivity.events.onclick.enable) {
            pJS.interactivity.el.addEventListener('click', function () {
                pJS.interactivity.mouse.click_pos_x = pJS.interactivity.mouse.pos_x; pJS.interactivity.mouse.click_pos_y = pJS.interactivity.mouse.pos_y; pJS.interactivity.mouse.click_time = new Date().getTime();
                if (pJS.interactivity.events.onclick.enable) {
                    switch (pJS.interactivity.events.onclick.mode) {
                        case 'push': if (pJS.particles.move.enable) pJS.fn.modes.pushParticles(pJS.interactivity.modes.push.particles_nb, pJS.interactivity.mouse); else { if (pJS.interactivity.modes.push.particles_nb == 1) pJS.fn.modes.pushParticles(pJS.interactivity.modes.push.particles_nb, pJS.interactivity.mouse); else if (pJS.interactivity.modes.push.particles_nb > 1) pJS.fn.modes.pushParticles(pJS.interactivity.modes.push.particles_nb); } break;
                        ...
                        case 'repulse': pJS.tmp.repulse_clicking = true; pJS.tmp.repulse_count = 0; pJS.tmp.repulse_finish = false; setTimeout(function () { pJS.tmp.repulse_clicking = false; }, pJS.interactivity.modes.repulse.duration * 1000); break;
                    }
                }
            });
        }
    ...
    pJS.fn.vendors.createSvgImg = function (p) {
        var svgXml = pJS.tmp.source_svg, rgbHex = /#([0-9A-F]{3,6})/gi, coloredSvgXml = svgXml.replace(rgbHex, function (m, r, g, b) {
            var color_value;
            if (p.color.rgb) color_value = 'rgba(' + p.color.rgb.r + ',' + p.color.rgb.g + ',' + p.color.rgb.b + ',' + p.opacity + ')';
            else color_value = 'hsla(' + p.color.hsl.h + ',' + p.color.hsl.s + '%,' + p.color.hsl.l + '%,' + p.opacity + ')';
            return color_value;
        });
    ...
    pJS.fn.vendors.draw = function () {
        // *** FPS FIX IMPLEMENTATION ***
        // We use delta time to ensure this specific instance respects the limit
        // regardless of the global requestAnimationFrame speed.

        var now = Date.now();
        var fps_limit = pJS.fps_limit;
        var elapsed = now - pJS.last_draw_time;
        ...
        if (fps_limit > 0 && elapsed < interval) {
            return;
        }

        pJS.last_draw_time = now - (elapsed % interval);
...
window.particlesJS = function (tag_id, params) {
    ...
    canvas_el.style.width = "100%"; canvas_el.style.height = "100%";
    var canvas = document.getElementById(tag_id).appendChild(canvas_el);
    if (canvas != null) { pJSDom.push(new pJS(tag_id, params)); }
};

/* -----------------------------------------------
 * Config Generation (Optimized to reduce code)
 * ----------------------------------------------- */

function createConfig(fpsLimit, numParticles, density, color, shapeType, opacity, size, linkedColor, moveSpeed, outMode) {
    return {
        "fpsLimit": fpsLimit,
        "particles": {
            "number": { "value": numParticles, "density": { "enable": true, "value_area": density } },
            "color": { "value": color },
            "shape": { "type": shapeType, "stroke": { "width": 0, "color": "#000" }, "polygon": { "nb_sides": 3 } },
            "opacity": { "value": opacity, "random": false, "anim": { "enable": false } },
            "size": { "value": size, "random": false, "anim": { "enable": false } },
            "line_linked": { "enable": true, "distance": 600, "color": linkedColor, "opacity": 0.4, "width": 5 },
            "move": { "enable": true, "speed": moveSpeed, "direction": "none", "random": false, "straight": false, "out_mode": outMode || "bounce", "bounce": false, "attract": { "enable": false } }
        },
        "interactivity": {
            "detect_on": "canvas",
...
(function () {
    ...
    // Define specific FPS limits for different layers
    // High FPS for sharp top layers, lower FPS for blurred backgrounds for performance
    let fpsHigh = 20;
    let fpsLow = 10;

    const configs = {
        // Layer 1: Blue Circles, High FPS
        'particles-js': createConfig(fpsHigh, 10, 600, "#1B79DE", "circle", 0.8, 5, "#175cff", 5, "bounce"),

        // Layer 2: Pink Circles, High FPS
        'particles-js2': createConfig(fpsHigh, 3, 600, "#C0007E", "circle", 0.8, 5, "#f024f6", 5, "bounce"),

        // Layer 3: Pink Circles, Lower FPS (Blurred)
        'particles-js3': createConfig(fpsLow, 7, 550, "#C0007E", "circle", 1, 7, "#C0007E", 4, "bounce"),

        // Layer 4: Blue Circles, Lower FPS (Blurred)
        'particles-js4': createConfig(fpsLow, 3, 550, "#1B79DE", "circle", 1, 7, "#175cff", 4, "bounce"),

        // Layer 5 & 6 (Missing in original JS but present in HTML - giving defaults)
        // 'particles-js5': createConfig(fpsLow, 5, 550, "#1B79DE", "circle", 0.5, 12, "#175cff", 3, "bounce"),
        // 'particles-js6': createConfig(fpsLow, 5, 550, "#C0007E", "circle", 0.5, 12, "#C0007E", 3, "bounce"),

        // // Layer 7: Blue Polygons, Low FPS (Very Blurred)
        // 'particles-js7': createConfig(fpsLow, 6, 600, "#1B79DE", "circle", 1, 16, "#175cff", 5, "bounce"),

        // Layer 8: Pink Polygons, Low FPS (Very Blurred)
        'particles-js8': createConfig(fpsLow, 3, 600, "#C0007E", "circle", 1, 16, "#C0007E", 5, "bounce")
    };

    const getScaleForWidth = function (w) {
        if (w <= 480) return 1.5;
    ...
    const adjustConfig = function (baseCfg, scale, w) {
        const cfg = JSON.parse(JSON.stringify(baseCfg)); // Deep copy
        try {
            const baseNumber = cfg.particles.number.value;
            const baseDensity = cfg.particles.number.density.value_area;

            let adjustedNumber = Math.max(1, Math.round(baseNumber * scale));
            let densityArea = Math.round(baseDensity * Math.max(1, 1 / scale));

            if (w <= 480) { densityArea = 400; adjustedNumber = Math.max(2, Math.floor(baseNumber / 2)); }

            cfg.particles.number.value = adjustedNumber;
            cfg.particles.number.density.value_area = densityArea;
        } catch (e) { console.warn('adjustConfig error', e); }
        return cfg;
    };

    const initResponsiveParticles = function () {
        const w = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
        const scale = getScaleForWidth(w);

        // Clear existing instances
        if (window.pJSDom && window.pJSDom.length > 0) {
            window.pJSDom.forEach(p => {
                cancelAnimationFrame(p.pJS.fn.drawAnimFrame);
            });
            window.pJSDom = [];
        }
        ...
        for (const id in configs) {
            ...
            try {
                const cfg = adjustConfig(configs[id], scale, w);
                particlesJS(id, cfg);
            } catch (e) { console.warn('particlesJS init failed for', id, e); }
        }
    };

    let resizeTimer;
    if (document.readyState === 'complete' || document.readyState === 'interactive') {
        setTimeout(initResponsiveParticles, 0);
    } else {
        window.addEventListener('DOMContentLoaded', initResponsiveParticles);
    }

    window.addEventListener('resize', function () {
        clearTimeout(resizeTimer);
        resizeTimer = setTimeout(initResponsiveParticles, 100);
    });
